...
import { getAdcode, getWeather, getTq } from '@/api'
...
const getWeatherData = () => {
  ...
  getTq()
    .then((res) => {
      console.log(res)
      if (res.code == 200) {
        weatherData.adCode.city = res.result.city.name
        console.log(weatherData.adCode.city)
        weatherData.weather = {
          weather: res.result.condition.condition,
          temperature: res.result.condition.temp,
          winddirection: res.result.condition.windDir,
          windpower: res.result.condition.windLevel
        }
